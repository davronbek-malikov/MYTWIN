...
def _classify(category: str, data: dict) -> str:
    ...
    cat = category.lower()
    if t in _INCOME_TYPES or cat in _INCOME_CATEGORIES:
        return "income"
    if t in _EXPENSE_TYPES or "xarajat" in cat or cat in (
...
async def generate_report(user_id: int, period: str = "month") -> str:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            f"SELECT category, data, description, created_at::text FROM entries "
            f"WHERE user_id = $1 {period_sql} ORDER BY created_at DESC",
            user_id,
        )
    ...
    if income_rows:
        lines.append(f"💰 *Income: {c}{income_total:,.2f}*")
        for date, desc, amt, cs in income_rows[:15]:
            lines.append(f"  • {date}  {desc}  {cs}{amt:,.2f}" if amt else f"  • {date}  {desc}")
        lines.append("")

    if expense_rows:
        lines.append(f"💸 *Expenses: {c}{expense_total:,.2f}*")
        for date, desc, amt, cs in expense_rows[:15]:
            lines.append(f"  • {date}  {desc}  {cs}{amt:,.2f}" if amt else f"  • {date}  {desc}")
        lines.append("")
    ...
    if income_rows or expense_rows:
        ...
        sign = "+" if net >= 0 else ""
        em   = "📈" if net >= 0 else "📉"
        lines.append(f"{em} *Net: {sign}{c}{net:,.2f}*")
